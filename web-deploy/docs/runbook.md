...
Incident-response handbook for the VarLens Concept Pilot. Per scenario: symptom, steps, verification, escalation.

> **Standing up a pilot from scratch?** That's a happy-path bring-up, not an incident — see [`/DEPLOY.md`](../../DEPLOY.md) instead. This runbook starts at "the server is already provisioned and something is wrong (or maintenance is due)."
