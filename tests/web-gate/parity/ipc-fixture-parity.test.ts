...
function renderReport(report: IpcParityReport): string {
  const lines = [
    '# Electron Postgres vs Web Postgres IPC Parity Report',
    '',
    ...
    '',
    '| IPC area | Status | Electron Postgres hash | Web Postgres hash | Operations | Duration ms |',
    '| --- | --- | --- | --- | ---: | ---: |'
  ]
...
describe.skipIf(!SHOULD_RUN || !existsSync(ELECTRON_BUILD) || !HAS_PG)(
  'Electron Postgres vs Web Postgres IPC parity E2E',
  () => {
    test('validates all 23 IPC areas against fixture-backed Electron Postgres and web Postgres runtimes', async () => {
...
describe.skipIf(SHOULD_RUN && existsSync(ELECTRON_BUILD) && HAS_PG)(
  'Electron Postgres vs Web Postgres IPC parity skipped notice',
  () => {
