...
| ------------------------------------------- | ---- | ------------------- | ----------------- | ----- |
| Routine: Restore Drill                      | C    | 2026-05-07 (KW 19)  | PASS (after fix)  | Initially FAILED. Three real bugs found and fixed in `scripts/restore-drill.sh`: (1) `sudo ls -1 .../varlens-*.dump` could not expand the glob because `restic restore` writes the target with mode 0700 — `chmod -R o+rX` after restore so the deploy user's shell can traverse; (2) `PROTOCOL_FILE` defaulted to `.internalplanning/restore-log.md` (typo, no such directory) → repo-root-anchored default `.planning/web/operations/restore-log.md`; (3) protocol parent dir was never created → added `mkdir -p`. Drill now writes a passing entry to `.planning/web/operations/restore-log.md`. |
...
2. **`scripts/restore-drill.sh` PROTOCOL_FILE typo** — defaulted to `.internalplanning/restore-log.md` (no such dir). Fixed: repo-root-anchored `.planning/web/operations/restore-log.md`, with `mkdir -p` of the parent.
