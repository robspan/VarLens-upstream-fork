...
Status: first transform layer implemented.

The transformation layer turns public or cached source artifacts into small VarLens-ready fixtures with known expected results.
...
- `prepare-fixtures.mjs` exists and supports `copy`, `copy-many`, `zip`, and `vcf-to-varlens-json` transforms.
...
- `transform-vcf-subset.mjs` and `transform-longread-bundle.mjs` exist for future public-source expansion.
