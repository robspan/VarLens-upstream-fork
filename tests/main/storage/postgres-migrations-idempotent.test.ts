...
describe.skipIf(!RUN)('Postgres migrations: real-instance idempotency', () => {
  ...
  it('runs all migrations end-to-end and produces a non-empty schema', async () => {
    ...
    // Asserting names alone is not enough — QA consensus (6/10 agents on
    // commit 9e1b7e68) flagged that a future careless edit could flip
    // is_active/must_change_password back to INTEGER, drop the role CHECK,
    // or drop NOT NULL on password_hash, and this test would still pass.
    ...
    // Role CHECK must enumerate exactly admin + user — same enum as
    // SQLite migrations.ts v12. The constants module (Phase 2 #2)
    // becomes the cross-backend source of truth; this test pins the
    // Postgres side until that lands.
