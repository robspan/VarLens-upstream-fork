...
describe('db-seam gate', () => {
  test('no new files import getDatabaseService / getDbPool from a definer module', () => {
    ...
    expect(
      newViolators,
      newViolators.length
        ? `New files import the loophole symbols. Either inject StorageSession instead, or add the file to ALLOWLIST_LOOPHOLE_IMPORTERS in this test (and document why in the PR). Context: .planning/web/phase1-gate-tests.md §db-seam\n  ${newViolators.join('\n  ')}`
