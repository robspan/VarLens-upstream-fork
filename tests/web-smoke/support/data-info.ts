...
export function attachBedRegionFilterThroughDataInfo(
  ...
  cy.wait('@regionCreateApi', { timeout: 30000 }).then((interception) => {
    expect(interception.response?.statusCode ?? 0, 'region file create status').to.be.within(200, 299)
  ...
  cy.wait('@dataInfoSaveApi', { timeout: 30000 }).then((interception) => {
    expect(interception.response?.statusCode ?? 0, 'case metadata save status').to.be.within(200, 299)
  })
...
export function expectBedRegionFilterPersisted(attachment: BedRegionFilterAttachment, regionName: string): void {
    ...
    cy.contains(/Region filter \(BED\)/i)
      ...
      .should(($field) => {
        ...
        const metadata = $field
          ...
          .map((_idx, el) => `${el.getAttribute('title') ?? ''} ${el.getAttribute('aria-label') ?? ''}`)
          .get()
