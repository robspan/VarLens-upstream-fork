 ...
 *
 *   __APP_PATH_PREFIX__   Caddy mounts the app under e.g. `/varlens`,
 *                         so the page's fetch() and post-login redirect
 *                         must include the prefix. Sourced from the
 *                         `APP_PATH_PREFIX` env (matches the same name
 *                         used by Caddy + the renderer build).
