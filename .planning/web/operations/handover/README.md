...
1. **CLAUDE.md** — replace with the post-handover snapshot:
   ```bash
   cp .planning/web/handover/CLAUDE.post-handover.md CLAUDE.md
   git add CLAUDE.md && git commit -m "docs(claude): drop fork-specific override on handover"
   diff CLAUDE.md .planning/web/handover/CLAUDE.post-handover.md   # must be empty
   ```
...
If all three pass and `diff CLAUDE.md .planning/web/handover/CLAUDE.post-handover.md` is empty, the mechanical handover is done.
