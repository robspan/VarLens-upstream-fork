...
- [x] Add failing tests in `tests/web-gate/auth-origin.test.ts` for
  `Sec-Fetch-Site: cross-site`, `none`, `same-origin`, `same-site`, missing
  Fetch Metadata plus valid Origin, and missing both signals.
- [x] Implement a small helper in `src/web/server/auth.ts` that returns an
  allow/reject decision for unsafe API requests.
- [x] Wire the helper into the existing preHandler.
- [x] Run `npx vitest run --project web-gate tests/web-gate/auth-origin.test.ts`.

### Package B: Server-Path Removal

- [x] Add failing tests that raw absolute paths reject even when `NODE_ENV=test`
  and even if `VARLENS_WEB_ALLOW_SERVER_PATH_IMPORT=1`.
- [x] Convert web success-path tests to call `stageExistingFileUpload(...)` and
  pass the returned `web-upload:` ref.
- [x] Make `serverPathImportDisabled()` always return `true` and update comments
...
- [x] Update `tests/web-gate/user-id-schema.test.ts` comments to remove
...
- [x] Update `.planning/web/completed/phase1-execution-plan.md` to keep
  `auth:createUser` disabled until row-level scoping exists.
- [x] Run `npx vitest run --project web-gate tests/web-gate/user-id-schema.test.ts`.

### Package D: Override Seam Guard

- [x] Add a static expected override module set to
  `tests/web-gate/handler-seam.test.ts`.
- [x] Assert the discovered `build*Overrides` route modules equal that set.
- [x] Run `npx vitest run --project web-gate tests/web-gate/handler-seam.test.ts`.
...
- [x] Run `rg -n "VARLENS_WEB_ALLOW_SERVER_PATH_IMPORT|server-path import is enabled|routes an enabled absolute server path" src`.
- [x] Run `make agent-check` if authored source structure changed.

`make agent-check` was executed after implementation. It still fails on
pre-existing oversized renderer component growth outside this narrow hardening
scope: `CohortTable.vue`, `ImportWizard.vue`, and `VariantTable.vue`.
