 ...
 * only asserted specific migration names. QA consensus (6/10 agents on
 * commit 9e1b7e68) flagged this as misleading: dropping a new
 ...
 *   - the Phase 2 #1 migration (0007 / create_users_and_settings) is
 *     present, also containing the table DDL it claims to ship
 */
...
describe('postgres migration registration', () => {
  ...
  it('registers the users + database_settings migration (Phase 2 #1)', () => {
    const m = POSTGRES_MIGRATIONS.find((x) => x.version === '0007')
    expect(m, 'migration 0007 must be registered').toBeDefined()
    expect(m!.name).toBe('create_users_and_settings')
    // Pin SQL content so an empty/stub file can't slip past — the Phase
    // 2 plan claims the migration creates these specific tables.
    expect(m!.sql, 'migration 0007 must declare the users table').toMatch(/CREATE TABLE.+users/i)
    expect(m!.sql, 'migration 0007 must declare the database_settings table').toMatch(
