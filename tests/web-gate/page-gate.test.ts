...
describe('web page gate', () => {
  ...
  test('does not expose platform auth paths in local mode', async () => {
    const app = fastify()
    try {
      registerPageGate(app, { appPathPrefix: '/varlens' })
      app.setNotFoundHandler(async (_request, reply) => {
      ...
      for (const url of ['/auth/platform/start', '/auth/platform/callback']) {
        const response = await app.inject({ method: 'GET', url })
        expect(response.statusCode, url).toBe(302)
        expect(response.headers.location, url).toMatch(/^\/varlens\/login\?next=/)
        expect(response.body, url).not.toContain('SPA shell')
      }
    } finally {
      await app.close()
    }
