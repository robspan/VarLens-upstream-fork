...
Status: first manifest-backed parity E2E harness implemented.
...
The manifest-backed E2E currently lives in `tests/web-gate/parity/data-manifest-parity.test.ts`.
A shared `BackendDriver` helper remains a cleanup follow-up once another parity scenario needs the
same harness shape.
...
The implemented first parity E2E milestone covers:
