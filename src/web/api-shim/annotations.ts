...
export const createAnnotationsApi = (): AnnotationsDomainContract => ({
  getGlobal: (chr, pos, ref, alt) => httpInvoke('/api/annotations/getGlobal', [chr, pos, ref, alt]),
  ...
  getPerCase: (caseId, variantId) => httpInvoke('/api/annotations/getPerCase', [caseId, variantId]),
  ...
  batchGet: (caseId, variantKeys) => httpInvoke('/api/annotations/batchGet', [caseId, variantKeys])
})
