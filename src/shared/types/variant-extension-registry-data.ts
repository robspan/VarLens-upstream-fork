/**
 * Single source of truth for variant extension tables — data module.
 *
 * This module holds the pure data + type definitions for the variant
 * extension registry so BOTH the main process and the renderer can import
 * it. SQL-emitting helpers (buildExtensionJoinClauses,
 * buildExtensionExistsClauses, translateExtensionFilter) live in
 * `src/main/database/variant-extension-registry.ts` which re-exports
 * everything from this module.
 *
 * The renderer CANNOT import from `src/main/…` due to the Electron process
 * boundary, so anything shared between main and renderer must live under
 * `src/shared/`.
 *
