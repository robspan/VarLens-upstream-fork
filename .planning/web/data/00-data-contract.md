...
VarLens has importers, but the test-data story is not yet expressed as a reproducible data contract. A developer should not need to know what a clinical geneticist expects from a VCF. They should be able to read a manifest, run a command, and compare normalized data.

## Fixture Unit

Each fixture must have one manifest entry with this shape:
