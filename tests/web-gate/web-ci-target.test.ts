...
describe('web CI target wiring', () => {
  ...
  test('web report treats VARLENS_WEB=1 as full parity mode', () => {
    ...
    expect(runner).toContain('## Abstract')
    expect(runner).toContain('## Validation Scope')
    expect(runner).toContain('## Methodology')
    expect(runner).toContain('## IPC Traceability Matrix')
    expect(runner).toContain('## Domain Data Parity')
    expect(runner).toContain('## Limitations')
    expect(runner).toContain('## Conclusion')
