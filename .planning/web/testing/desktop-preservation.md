...
- `handler-storage-callgraph.test.ts` — overlap with `tests/web-gate/db-seam.test.ts`. Add only on rule-of-three.

## Lifecycle

Both tests retire when the StorageSession refactor is done. `pool-vs-main-routing` becomes moot once Postgres is the production web path (no Piscina pool).
