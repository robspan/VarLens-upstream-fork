...
describe('user-id-schema gate', () => {
  ...
  test('every non-exempt table has user_id NOT NULL DEFAULT 1, modulo the snapshot', () => {
    ...
    expect(
      missing,
      missing.length
        ? `Tables added without user_id. Add 'user_id INTEGER NOT NULL DEFAULT 1' to the migration, OR if the table is shared/junction/reference data, add it to EXEMPT_TABLES in this test. Context: .planning/web/phase1-gate-tests.md §user-id-schema\n  ${missing.join('\n  ')}`
