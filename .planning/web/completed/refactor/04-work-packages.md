...
Depends on: WP1  
Status: completed for the cleanup target set; variants, cohort reads, cohort status, database startup, auth startup, import error behavior, and explicit unsupported web responses are covered.  
...
Depends on: WP3  
Status: completed for the cleanup target set; auth management routes, service-backed accounts flag, session revalidation, stale-session behavioral tests, and API Origin checks are implemented.  
...
Depends on: WP2  
Status: completed for current web mode; database picker and import entry points are gated, browser-incompatible exports are hidden via web capabilities and fail explicitly when called directly, native folder actions are no-op browser actions, and remaining product support is backlog.  
...
Status: completed for the cleanup pass  
