...
| ID | Title | Audit ref | Rationale |
|---|---|---|---|
| **QW-4** | `npm audit fix` for the one moderate-severity transitive (`qs` under `pg`). Verify `npm audit --omit=dev` returns **0 moderate** (target: 0 critical / 0 high / 0 moderate; current pre-fix state is 5 low + 1 moderate per `npm audit --omit=dev`). **Do not** run `npm audit fix --force` — that would upgrade `pdbe-molstar` to 3.1.3 (breaking change in the renderer's 3D-structure view), which is out of scope per Non-goals. The 5 `low` entries are the `elliptic` chain via `pdbe-molstar`; Dependabot may surface the same chain as `high` because it scores `elliptic` upstream rather than the in-bundle transitive. Both views are accepted: PR-3 only fixes `qs`. | §3.2 | Open advisory in shipping graph. |
...
- **`pdbe-molstar` 3.x major upgrade (elliptic vulnerability chain)** — separate dedicated phase. Surfaced after Phase 1 commit when GitHub Dependabot reported 1 high alongside the `qs` moderate that QW-4 already covers. The "high" maps to `elliptic` (GHSA-848j-6mx2-7j84) reaching the bundle via `pdbe-molstar → crypto-browserify → browserify-sign → elliptic`. `npm audit fix --force` resolves the chain by installing `pdbe-molstar@3.1.3`, flagged by npm as a breaking change in the renderer's 3D-structure view. QW-4 in Phase 1 stays scoped to the **non-breaking `qs` fix**; the pdbe-molstar major upgrade gets its own spec with UAT for the protein viewer.
...
3. **QW-4 evidence:** `npm audit --omit=dev` returns **0 moderate** (today: 5 low + 1 moderate; the 5 `low` are the `elliptic` chain via `pdbe-molstar` and are explicitly out of Phase 1 scope per Non-goals). GitHub Dependabot's parallel "1 high" report tracks the same `elliptic` chain at upstream-package severity; that does not block Phase 1.
